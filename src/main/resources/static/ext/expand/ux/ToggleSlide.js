 ...
 *
 * The Initial Developer of the Original Code is Sean McDaniel
 * <sean@mcdconsulting.com> Copyright(c) 2010, http://www.mcdconsultingllc.com
 ...
 */
Ext.define('Ming.expand.ux.ToggleSlide', {
    ...
    renderTpl: ['<div class="holder">', '<label class="{onLabelCls}">',
        '<span>{onText}</span>', '</label>',
        '<label class="{offLabelCls}">', '<span>{offText}</span>',
        '</label>', '</div>'],
    ...
    resize: function () {
        ...
        if (me.resizeHandle) {
            var min = (onlabel.getWidth() < offlabel.getWidth()) ? onlabel
                .getWidth() : offlabel.getWidth();
            handle.setWidth(min);
        }

        if (me.resizeContainer) {
            var max = (onlabel.getWidth() > offlabel.getWidth()) ? onlabel
                .getWidth() : offlabel.getWidth();
        ...
        var rightside = me.rightside = container.getWidth() - handle.getWidth()
            - me.diff;
    ...
    moveHandle: function (on, callback) {
        var me = this, runner = new Ext.util.TaskRunner(), to = on
            ? me.rightside
            : 0;
    ...
    onDragEnd: function (e) {
        var me = this, hc = (me.handle.getLeft(true) + me.handle.getRight(true))
            / 2, cc = (me.el.getLeft(true) + me.el.getRight(true)) / 2, next = hc > cc;
    ...
    toggle: function () {
        ...
        if (me.fireEvent('beforechange', me, next) !== false) {
            ...
            me.moveHandle(me.state, Ext.bind(me.fireEvent, me, ['change', me,
                me.getValue()]));
        } else {
...
Ext.define('Ext.ux.toggleslide.Thumb', {
    ...
    /**
     * @private
     * @property {Number} topThumbZIndex The number used internally to
     *           set the z index of the top thumb (see promoteThumb for
     *           details)
     */
    topZIndex: 10000,

    /**
     * @cfg {Ext.slider.MultiSlider} slider (required) The Slider to
     *      render to.
     */
    ...
    constructor: function (config) {
        ...
        /**
         * @property {Ext.slider.MultiSlider} slider The slider this
         *           thumb is contained within
         */
        Ext.apply(me, config || {}, {
            cls: Ext.baseCSSPrefix + 'toggle-slide-thumb',

            /**
             * @cfg {Boolean} constrain True to constrain the
             *      thumb so that it cannot overlap its siblings
             */
            constrain: false
        });
    ...
    /**
     * @private Bring thumb dom element to front.
     */
    ...
    /**
     * @private Send thumb dom element to back.
     */
    sendToBack: function () {
