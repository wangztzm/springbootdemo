Ext.define('Ming.view.main.login.LockScreen', {
  ...
  alias : 'widget.lockscreen',
    ...
    initComponent: function () {
        ...
        this.t_usercode = Ext.create('Ext.form.field.Text', {
            ...
            emptyText: '用户名称',
            width: 250,
            listeners: {specialkey: me.onUserNameEnterKey}
        });
        ...
        var btns = {
            xtype: 'container', layout: 'table', defaults: {margin: 20}, items: [
    ...
    beforeShow: function () {
        ...
        this.l_username.getEl().setHtml('<h2><font  color="#ffffff"><span style="letter-spacing:2mm;text-shadow: 3px 3px 3px rgba(42, 42, 42, 0.75);">' + cfg.sub.username + '</span></font></h2>');
        this.t_usercode.setValue(cfg.sub.username);
    ...
    validate: function (invalidate) {
        ...
        if (Ext.isEmpty(username)) {
            EU.toastErrorInfo('用户名不能为空');

            return;
        }
        if (Ext.isEmpty(password)) {
            EU.toastErrorInfo('密码不能为空');
        ...
        EU.RS({
            url: url, scope: this, params: userinfo, callback: function (result) {
                if (result.success) {
                    ...
                    if (!sameUser) {
                        ...
                        if (Ext.msgWin) Ext.msgWin.initStart();
                    }
                ...
                switch (result.data) {
                    ...
                    case '7': {
                        EU.showMsg({
                            title: '提示信息', message: '当前用户已经在线，确定强制登录吗？', option: 1, callback: function (rt) {
